...
class AllCNNC(Chain):
    ...
    def __init__(self):
        ...
        super(AllCNNC, self).__init__(
            ...
            l1=ConvBNAct(
                conv=F.Convolution2D(in_channels=3, out_channels=96, ksize=ksize, stride=1, pad=pad),
                bn=bn,
                act=act
            ),

            l2=ConvBNAct(
                conv=F.Convolution2D(in_channels=96, out_channels=96, ksize=ksize, stride=1, pad=pad),
                bn=bn,
                act=act
            ),

            l3=ConvBNAct(
                conv=F.Convolution2D(in_channels=96, out_channels=96, ksize=ksize, stride=2, pad=pad),
                bn=bn,
                act=act
            ),

            l4=ConvBNAct(
                conv=F.Convolution2D(in_channels=96, out_channels=192, ksize=ksize, stride=1, pad=pad),
                bn=bn,
                act=act
            ),

            l5=ConvBNAct(
                conv=F.Convolution2D(in_channels=192, out_channels=192, ksize=ksize, stride=1, pad=pad),
                bn=bn,
                act=act
            ),

            l6=ConvBNAct(
                conv=F.Convolution2D(in_channels=192, out_channels=192, ksize=ksize, stride=2, pad=pad),
                bn=bn,
                act=act
            ),
            ...
            l8=ConvBNAct(
                conv=F.Convolution2D(in_channels=192, out_channels=192, ksize=1, stride=1, pad=0),
                bn=bn,
                act=act
            ),
            ...
            l9=ConvBNAct(
                conv=F.Convolution2D(in_channels=192, out_channels=10, ksize=1, stride=1, pad=0),
                bn=bn,
                act=act
            ),
        )
